...
//--------------  getlist data fetch start -------------
  ...
  // ------ init function call start ------
    ...
    commonFunction(){
      ...
      // getlist data
      this.getlist('company/getlist');

      if( this.parms_action_name == 'edit'){
        // form submit api edit
        this.form_api = 'company/return_edit/'+this.parms_action_id;
      }else{
        // form submit api add
        this.form_api = 'company/return_add'
      }
        ...
        (res:any) => {
          ...
          if(res.return_status > 0){
            this.model = {
              ...
              industry_type: res.return_data.industry_type_id,
              company_type: res.return_data.company_type_id,
              profile: res.return_data.company_logo,
            ...
            // multiple contact
            if(res.return_data.contact.length > 0){
              this.model.pages = res.return_data.contact;
            }else{
              // contact multiple
              this.model.pages = [
                {
            ...
            // multiple address
            if(res.return_data.address.length > 0){
              this.model.pages_address = res.return_data.address;
            }else{
              // address multiple
              this.model.pages_address = [
                {
      ...
      (res:any) => {
        ...
        if(res.return_status > 0){
          ...
          if(_name == 'return_getState'){
            _colon_item.stateList = res.return_data.state;
          }else{
            _colon_item.cityList = res.return_data.city;
          }
        }
    },
    errRes => {
          ...
          if(resData.country.list){
            resData.country.list.forEach(value => {
              if(value.id == resData.country.default){
                this.model.pages_address.forEach(value1 => {
                  value1.country_id = value.id;
                  this.default_country_id = value.id;
                  this.contactByCompany(this.default_country_id , 'return_getState', '' );
                });
              }
            });
          }
  ...
  reloadPage(){
    if( this.parms_action_name == 'add'){
      ...
      this.model = {
        ...
        industry_type: this.allEditData.return_data.industry_type_id,
        company_type: this.allEditData.return_data.company_type_id,
        profile: this.allEditData.return_data.company_logo,
      ...
      // multiple contact
      if(this.allEditData.return_data.contact.length > 0){
        this.model.pages = this.allEditData.return_data.contact;
      }else{
        // contact multiple
        this.model.pages = [
          {
      ...
      // multiple address
      if(this.allEditData.return_data.address.length > 0){
        this.model.pages_address = this.allEditData.return_data.address;
      }else{
        // address multiple
        this.model.pages_address = [
          {
...
addItem(_items, _item){
  ...
  _items.push({"is_default":true,"country_id":this.default_country_id});
