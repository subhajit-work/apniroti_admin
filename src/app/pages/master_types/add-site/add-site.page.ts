...
//--------------  getlist data fetch start -------------
  ...
  // ------ init function call start ------
    ...
    commonFunction(){
      ...
      // getlist data
      this.getlist('site/getlist');

      if( this.parms_action_name == 'edit'){
        // form submit api edit
        this.form_api = 'site/return_edit/'+this.parms_action_id;
      }else{
        // form submit api add
        this.form_api = 'site/return_add'
      }
        ...
        (res:any) => {
          ...
          if(res.return_status > 0){
            this.model = {
              ...
              favicon : res.return_data.favicon,
              logo : res.return_data.logo,
              logo2 : res.return_data.logo2,
              ...
              cgst : res.return_data.cgst,
              sgst : res.return_data.sgst,
              igst : res.return_data.igst,
              email : res.return_data.email,
              alt_email : res.return_data.alt_email,
              phone : res.return_data.phone,
              alt_phone : res.return_data.alt_phone,
              razorpay_key : res.return_data.razorpay_key,
            ...
            // multiple social
            if(res.return_data.social && res.return_data.social.length > 0){
              this.model.social = res.return_data.social;
            }else{
              // social multiple
              this.model.social = [
                {
...
onSubmit(form:NgForm){
  ...
  if(this.fileValLogo) {
  // normal file upload
  fd.append(this.normalFileNameLogo, this.fileValLogo, this.fileValLogo.name);
  }else{
    fd.append(this.normalFileNameLogo, '');
  }

  if(this.fileValLogo2) {
  // normal file upload
  fd.append(this.normalFileNameLogo2, this.fileValLogo2, this.fileValLogo2.name);
  }else{
    fd.append(this.normalFileNameLogo2, '');
  }


  // fileValFabicon
  if(this.fileValFabicon) {
    // normal file upload
    fd.append(this.normalFileNameFabicon, this.fileValFabicon, this.fileValFabicon.name);
  }else{
    fd.append(this.normalFileNameFabicon, '');
  }
...
normalFileNameLogo;
normalFileNameLogo2;
normalFileNameFabicon;
normalFileUpload(event, _item, _name) {
  console.log('nomal file upload _item => ', _item);
  console.log('nomal file upload _name => ', _name);

  if(_name == 'logo'){
    this.fileValLogo =  event.target.files[0];
    _item =  event.target.files[0].name;
    this.normalFileNameLogo = _name;
  }else if(_name == 'logo2'){
    this.fileValLogo2 =  event.target.files[0];
    _item =  event.target.files[0].name;
    this.normalFileNameLogo2 = _name;
  }else{
    this.fileValFabicon =  event.target.files[0];
    _item =  event.target.files[0].name;
    this.normalFileNameFabicon = _name;
  }
}
fileCross(_item, _identifire){
  if(_identifire == 'logo'){
    this.model.logo = null;
    this.normalFileNameLogo = '';
  }else if(_identifire == 'logo2'){
    this.model.logo2 = null;
    this.normalFileNameLogo2 = '';
  }else{
    this.model.favicon = null;
    this.normalFileNameFabicon = '';
  }
}
  ...
  reloadPage(){
    if( this.parms_action_name == 'add'){
      ...
      this.model = {
        ...
        seo : this.allEditData.return_data.seo,
        favicon : this.allEditData.return_data.favicon,
        logo : this.allEditData.return_data.logo,
        logo2 : this.allEditData.return_data.logo2,
        ...
        cgst : this.allEditData.return_data.cgst,
        sgst : this.allEditData.return_data.sgst,
        igst : this.allEditData.return_data.igst,
        email : this.allEditData.return_data.email,
        alt_email : this.allEditData.return_data.alt_email,
        phone : this.allEditData.return_data.phone,
        alt_phone : this.allEditData.return_data.alt_phone,
        razorpay_key : this.allEditData.return_data.razorpay_key,
...
// getSeoName
getSeoName(_item){
  console.log('getSeoName _item >',  _item);
  if(_item){
    //edit data call
    this.editDataSubscribe = this.http.get('seo/return_generateSeoURL/post?name='+_item).subscribe(
      (res:any) => {
        this.editLoading = false;
