...
export class AppliedJobPage implements OnInit, OnDestroy {
  ...
  constructor(
    ...
    // ------ init function call start -------
      commonFunction(){
        ...
        this.headerUrlapi = 'job_applicant/header';
    
        // table list data url name
        this.listing_url = 'job_applicant/return_index';


        this.onHeaderData(this.headerUrlapi);
        
        // getlist data url name
        // this.getlist('job_applicant/getlist');
      ...
      onDownload(_identifier, _items, _item){
        ...
        if(_identifier == 'multiple'){
          this.itemsSubscribe = this.authService.globalparamsData.subscribe(res => {
            this.export_url = this.main_url+'/job_applicant/download_csv?id='+items_id_array.join()+'&token='+res.token;
          });
