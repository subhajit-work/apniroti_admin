...
@Component({
  selector: 'app-feed-count-list',
  templateUrl: './feed-count-list.page.html',
  styleUrls: ['./feed-count-list.page.scss'],
})

export class FeedCountListPage implements OnInit, OnDestroy {
  ...
  constructor(
    ...
    // ------ init function call start -------
      commonFunction(){
        ...
        this.headerUrlapi = 'feed_count/header';
    
        // table list data url name
        this.listing_url = 'feed_count/return_index';
    ...
    // ------ export function call start ------
      export_url;
      onDownload(_identifier, _items, _item){
        ...
        if(_identifier == 'multiple'){
          this.itemsSubscribe = this.authService.globalparamsData.subscribe(res => {
            this.export_url = this.main_url+'/student/download_csv?id='+items_id_array.join()+'&token='+res.token;
          });
          ...
          /* this.deleteListSubscribe = this.http.get('student/download_csv?id='+items_id_array.join()).subscribe(
            (res:any) => {
              if(res.return_status > 0){
                this.checkedList = [];
                // this.commonUtils.presentToast('success', res.return_message);
