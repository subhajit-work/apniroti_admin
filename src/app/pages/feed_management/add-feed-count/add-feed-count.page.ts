...
@Component({
  selector: 'app-add-feed-count',
  templateUrl: './add-feed-count.page.html',
  styleUrls: ['./add-feed-count.page.scss'],
})

export class AddFeedCountPage implements OnInit, OnDestroy {
...
//--------------  getlist data fetch start -------------
  ...
  // ------ init function call start ------
    ...
    commonFunction(){
      ...
      // getlist data
      this.getlist('feed_count/getlist');

      if( this.parms_action_name == 'edit'){
        // form submit api edit
        this.form_api = 'feed_count/return_edit/'+this.parms_action_id;
      }else{
        // form submit api add
        this.form_api = 'feed_count/return_add'
      }
        ...
        (res:any) => {
          ...
          if(res.return_status > 0){
            this.model = {
              ...
              no_of_feeds : res.return_data.no_of_feeds,
              feed_increase_count : res.return_data.feed_increase_count,
              feed_increase_day : res.return_data.feed_increase_day,
    ...
    (response:any) => {
      ...
      if(response.return_status > 0){
        ...
        if(this.clickButtonTypeCheck == 'Save'){
          ...
          this.router.navigate(['/feed-count-list']);
...
//----------- reload page start ------------
  reloadPage(){
    if( this.parms_action_name == 'add'){
      ...
      this.model = {
        no_of_feeds : this.allEditData.return_data.no_of_feeds,
        feed_increase_count : this.allEditData.return_data.feed_increase_count,
        feed_increase_day : this.allEditData.return_data.feed_increase_day,
