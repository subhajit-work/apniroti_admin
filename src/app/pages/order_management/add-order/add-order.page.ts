...
export class AddOrderPage implements OnInit, OnDestroy {
  constructor(
...
//--------------  getlist data fetch start -------------
  ...
  // ------ init function call start ------
    ...
    commonFunction(){
      ...
      // getlist data
      this.getlist('order_details/getlist');

      if( this.parms_action_name == 'edit'){
        // form submit api edit
        this.form_api = 'order_details/return_edit/'+this.parms_action_id;
      }else{
        // form submit api add
        this.form_api = 'order_details/admin_return_add'
      }
        ...
        (res:any) => {
          ...
          if(res.return_status > 0){
            this.model = {
              ...
              user_id: res.return_data.user_id,
              identifier : res.return_data.identifier,
              duration : res.return_data.duration,
              price : res.return_data.price,
    ...
    this.plt.ready().then(() => {
      ...
      this.getListSubscribe = this.commonUtils.getlistCommon(_getlistUrl).subscribe(
        resData => {
          ...
          this.userList = resData.user;
          this.durationList = resData.duration;
          this.typeList = resData.type;
  ...
  reloadPage(){
    if( this.parms_action_name == 'add'){
      ...
      this.model = {
        ...
        user_id : this.allEditData.return_data.user_id,
